...
class StockController extends Controller
{
    ...
    public function filter(Request $request)
    {
        ...
        if (Carbon::parse($fromDate) < Carbon::parse($toDate)) {
            $title = "Active products from " . Carbon::parse($fromDate)->format('M d, Y') . " to " . Carbon::parse($toDate)->format('M d, Y');
        } elseif (Carbon::parse($fromDate) == Carbon::parse($toDate)) {
            $title = "Active products of " . Carbon::parse($fromDate)->format('M d, Y');
        } else {
            $title = "Active products from " . Carbon::parse($toDate)->format('M d, Y') . " to " . Carbon::parse($fromDate)->format('M d, Y');
        }

        $stocks = Stock::with(['product'])
            ->when($fromDate, fn(Builder $query) => $query->whereDate('stock_date', '>=', $fromDate))
            ->when($toDate, fn(Builder $query) => $query->whereDate('stock_date','<=',$toDate))
            ->when(\request('search'), function (Builder $query) {
                $query->whereHas('product',fn(Builder $query) => $query->where('product_name','like','%'.\request('search').'%'));
            })
            ->orderByDesc('stock_date')
            ->sortable()
            ->paginate($row)
